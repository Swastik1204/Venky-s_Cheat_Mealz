...
import { fetchMenuCategories, upsertMenuCategory, addMenuItems, setMenuItems, renameMenuCategory, fetchAllOrders, updateOrder, nextOrderStatus, removeMenuItem, appendMenuItems as lowAppend, fetchAppearanceSettings, saveCategoriesOrder, fetchImagesByIds, fetchAppSettings, saveAppSettings, fetchDeliverySettings, saveDeliverySettings, sendWhatsAppInvoice, sendSMSInvoice, BRAND_LONG, saveBase64Image } from '../lib/data'
...
export default function Admin({ section = 'inventory' }) {
	...
	useEffect(() => {
		let active = true
		import('../lib/storeStatus').then(mod => mod.fetchStoreStatus()).then((s) => {
			if (!active) return
			setLiveEnabled(s.open !== false)
		}).catch((err) => {
	...
	useEffect(() => {
		...
		fetchImagesByIds(ids).then(map => {
			...
			console.warn('Failed to resolve image IDs', err)
		})
		return () => { active = false }
	}, [categories])
		...
		<div className="page-wrap py-6">
			...
			<div className="mb-6 flex items-center justify-end gap-3 text-xs">
				...
				<button className="btn btn-xs" onClick={async () => {
					...
					setLiveEnabled(next)
					try {
						const mod = await import('../lib/storeStatus')
						await mod.setStoreOpen(next)
					} catch { /* noop */ }
																			...
																			onClick={async () => {
																				try {
																					...
																					setCategories(prev => prev.map(cat => {
																						...
																						const items = cat.items.map((x, i) => i === idx ? { ...x, imageId: undefined } : x)
																						return { ...cat, items }
																					}))
																					// Persist change
																					const target = categories.find(cat => cat.id === c.id)
																					if (target) {
																						const items = target.items.map((x, i) => i === idx ? { ...x, imageId: undefined } : x)
																						await setMenuItems(c.id, items)
																					}
																					pushToast('Item image removed.', 'info')
																				} catch (er) {
																					pushToast(er.message || 'Failed to remove image', 'error')
																				}
																			}}
																		>Delete</button>
																	)}
