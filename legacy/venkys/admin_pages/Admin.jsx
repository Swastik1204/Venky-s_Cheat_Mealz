...
import { fetchMenuCategories, upsertMenuCategory, addMenuItems, setMenuItems, renameMenuCategory, fetchAllOrders, updateOrder, nextOrderStatus, removeMenuItem, appendMenuItems as lowAppend, fetchStoreStatus, setStoreOpen, fetchAppearanceSettings, saveCategoriesOrder, fetchImagesByIds, fetchAppSettings, saveAppSettings, sendWhatsAppInvoice, sendSMSInvoice, BRAND_LONG, saveBase64Image } from '../lib/data'
...
export default function Admin({ section = 'inventory' }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      fetchAppSettings(),
      import('../../lib/deliverySettings').then(m => m.fetchDeliverySettings()),
    ]).then((results)=>{
      const [a,b] = results
      if (!active) return
              ...
              <button className="btn btn-primary" disabled={appSettingsSaving} onClick={async ()=>{
                ...
                try {
                  ...
                  if (!Number.isNaN(lat) && !Number.isNaN(lng) && !Number.isNaN(rad)) {
                    const mod = await import('../../lib/deliverySettings')
                    await mod.saveDeliverySettings({ centerLat: lat, centerLng: lng, radiusKm: rad })
                  }
