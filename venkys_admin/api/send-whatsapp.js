...
export default async function handler(req, res) {
  ...
  const phoneNumberId = (process.env.WA_PHONE_NUMBER_ID || '').trim()
    ...
    if (!r.ok) {
      try {
        console.error('[send-whatsapp] WA error', JSON.stringify(r.data))
      } catch (err) {
        // Best-effort logging only; ignore JSON stringify failures
        console.error('[send-whatsapp] WA error (stringify failed)', err)
      }
