 ...
 * Also exported for manual use (e.g. stock low alerts).
 */
...
/**
 * Determine if this event should trigger an email alert
 */
function shouldEmailAlert(action, collectionName, metadata = {}) {
  if (metadata?.emailAlert) return true
  const key = `${collectionName}_${action}`
  if (EMAIL_WORTHY_EVENTS.has(key)) return true
  // Order cancellations / rejections
  if (collectionName === 'orders' && action === 'update') {
    const newStatus = metadata?.newStatus || metadata?.after?.status
...
export async function logChange({
  ...
  try {
    ...
    if (shouldEmailAlert(action, collectionName, metadata)) {
      const summary = metadata?.details || metadata?.reason || `${action} on ${collectionName}/${documentId}`
      sendLogEmail(
        `${collectionName}_${action}`,
        `[${action.toUpperCase()}] ${summary}`,
        { documentId, performedBy: performedBy || 'system', changedFields: metadata?.changedFields }
      )
    }
  } catch (error) {
