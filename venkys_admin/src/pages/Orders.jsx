...
export default function Orders() {
  ...
  async function verifyOtpAndDeliver(o, rawOtp) {
    if (!canHandlePlacedOrders) return
    if (!o || o.status !== 'placed') return
    if (!isDineInCod(o)) return
    if (!o.cashManagerOtp) {
      pushToast('Payment not yet generated by Cash Manager', 'warning')
      return
    }

    // Check expiry
    const generatedAt = o.cashManagerOtpGeneratedAt?.seconds ? new Date(o.cashManagerOtpGeneratedAt.seconds * 1000) : (o.cashManagerOtpGeneratedAt ? new Date(o.cashManagerOtpGeneratedAt) : null)
    if (generatedAt && (Date.now() - generatedAt.getTime() > 30 * 60 * 1000)) {
      pushToast('OTP has expired. Customer must regenerate it at Cash Manager.', 'error')
      return
    }

    // Check lock
    if (o.verificationLockedUntil) {
      const lockedUntil = o.verificationLockedUntil?.seconds ? new Date(o.verificationLockedUntil.seconds * 1000) : new Date(o.verificationLockedUntil)
      if (lockedUntil > new Date()) {
        pushToast('Too many failed attempts. Order locked for 5 minutes.', 'error')
        return
      }
    }
    ...
    if (!typed || typed !== expected) {
      const attempts = (o.verificationAttempts || 0) + 1
      const patch = { verificationAttempts: attempts }
      if (attempts >= 3) {
        patch.verificationLockedUntil = new Date(Date.now() + 5 * 60 * 1000)
        pushToast('3 failed attempts. Locked for 5 mins.', 'error')
      } else {
        pushToast(`Incorrect OTP. ${3 - attempts} attempts remaining.`, 'error')
      }
      await updateOrder(o.userId || null, o.id, patch, user?.email)
      return
    }
    ...
    try {
      ...
      await updateOrder(o.userId || null, o.id, {
        ...
        cashManagerOtp: null, // DELETE THE OTP
        payment: {
          ...(o.payment || {}),
          status: 'paid',
          collectedAt,
      ...
      pushToast('Payment verified & Order Delivered', 'success')
      setOtpModalOpen(false)
      setOtpModalOrder(null)
      setOtpValue('')
    } catch (err) {
      console.error('[Orders] OTP verify failed:', err)
  ...
  async function acceptOrder(o) { 
    ...
    try {
      ...
      if (isDineInCod(o) && !o.cashManagerOtpVerified) {
          ...
          <div className="modal-box z-[210]">
            ...
            <div className="mt-4">
              ...
              <p className="mt-2 text-xs opacity-70">
                Ask the customer to provide the OTP shown on the Cash Manager's app.
              </p>
            </div>
                      ...
                      {o.status === 'placed' && canHandlePlacedOrders && (
                        <div className="flex gap-1" onClick={(e)=> e.stopPropagation()}>
                          <button className="btn btn-xs btn-success" onClick={() => acceptOrder(o)} disabled={frozen} title={frozen ? 'Actions disabled for past orders' : 'Accept'}>
                            {isDineInCod(o) && !o.cashManagerOtpVerified ? 'Enter OTP' : 'Accept'}
                          </button>
