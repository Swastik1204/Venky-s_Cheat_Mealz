...
* **GitHub Actions Workflow** (`.github/workflows/deploy.yml`), job display names as of 2026-08-26 rename/consolidation pass:
  ...
  * `deploy-production` → **"Deploy to Production"**: on push to `prod`, executes atomic deployment:
    ...
    ```
    **This line was factually wrong from workflow creation until 2026-09-04** — the `--only` flag actually shipped had no `firestore` target, only the two `hosting:` entries. See the "10-day rules-deploy gap" entry below for what that caused and how it was found and fixed. Verify a claim like this against the real current `deploy.yml`, not against this file, before repeating it.
